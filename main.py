...
"""
Superset MCP Integration (FastMCP v2)
...
Updated for FastMCP v2 and exposed over HTTP at /mcp.
"""
...
async def make_api_request(
    ...
    async def make_request() -> httpx.Response:
        ...
        if method.lower() == "get":
            ...
            return await client.delete(endpoint, headers=headers, params=params)
        else:
...
async def superset_database_get_tables(
    ...
    """
    return await make_api_request(ctx, "get", f"/api/v1/database/{id}/tables/".replace("{id}", str(database_id)))
